...
class window {
  ...
  public:
    ...
    void Drawsquare(const int iX1, const int iY1, const int ilenght, const drawstyle dsStyle = FILLED);
    ...
    void DrawPolygon(const int* ipX, const int* ipY, const int iVertices, const drawstyle dsStyle);
    ...
    void DrawregPolygon(const int* iPX, const int* iPY, const int iVertices, const drawstyle dsStyle);

    // Draws a circle centered at (iX, iY) with a radius of iRadius.  Valid drawstyles 
    // are FRAME, FILLED, and INVERTED.
    void DrawCircle(const int iX, const int iY, const double iRadius, const drawstyle dsStyle = FILLED);
